...
export const GET = apiHandler<{ params: { id: string } }>(async (req, context) => {
  ...
  try {
    ...
    try {
      await addCredits(userId, 1, `Refund: channel-analytics failure for ${channelIdParam}`);
      logger.info("Successfully refunded user credit upon channel analytics failure", { userId, channelId: channelIdParam });
    } catch (refundError) {
      logger.error("CRITICAL CREDIT REFUND FAILURE: Failed to refund user credit upon channel analytics failure", {
        userId,
